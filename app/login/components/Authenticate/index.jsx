...
const Authenticate = () => {
  ...
  const { validation, error, noError, validateAt, resetError } =
    useFormValidation(shape);
  ...
  const handleVerifyOtp = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        '/api/verify-otp',
        { email, otp: otpValue, username, phone, mode: authMode },
        {
          headers: {
  ...
  const handleSendOtp = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        '/api/send-otp',
        { otpValue, username, phone, email, mode: authMode },
        {
          headers: {
            Authorization: 'Bearer mytoken',
            'Content-Type': 'application/json',
    ...
    <>
      <div className={clsx(dancingScript.className, 'text-4xl')}>
        <span className={`${isLoginMode ? 'text-shi_brown' : ''}`}>
          <button onClick={() => handleAuthMode(AUTH_MODE.login)}>Login</button>
        </span>{' '}
        /&nbsp;
        <span className={`${!isLoginMode ? 'text-shi_brown' : ''}`}>
      ...
      <form
        className="bg-white p-6 rounded-lg shadow-md w-full max-w-sm mt-4"
        ref={formRef}
      >
        {!isLoginMode ? (
          <div className="mb-1">
